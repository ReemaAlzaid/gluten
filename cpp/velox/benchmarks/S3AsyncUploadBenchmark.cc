...
std::optional<std::string> firstEnv(
...
bool defaultPathStyleAccess(const std::optional<std::string>& endpoint) {
  ...
  return contains(endpoint.value(), "localhost") ||
      contains(endpoint.value(), "127.0.0.1") ||
      !contains(endpoint.value(), "amazonaws.com");
}

std::optional<std::string> inferAwsRegion(
  ...
  if (s3Prefix == std::string::npos || awsSuffix == std::string::npos ||
      awsSuffix <= s3Prefix + 3) {
    return std::nullopt;
  }
...
class S3AsyncUploadBenchmark {
 public:
  S3AsyncUploadBenchmark() {
    ...
    VELOX_USER_CHECK(
        !bucket_.empty(),
        "Set GLUTEN_S3_BENCH_BUCKET or S3_BUCKET before running this benchmark.");

    objectPrefix_ =
        getEnvOrDefault("GLUTEN_S3_BENCH_PREFIX", "gluten-s3-benchmark");
    minPartSize_ =
        getEnvOrDefault("GLUTEN_S3_BENCH_MIN_PART_SIZE", kDefaultMinPartSize);
    maxConcurrentUploadNum_ =
        getEnvOrDefault("GLUTEN_S3_BENCH_MAX_CONCURRENCY", "4");
    ...
    filesystems::initializeS3(
        ...
        getEnv("GLUTEN_S3_BENCH_LOG_LOCATION")
            .value_or(std::string(kDefaultLogLocation)));
  }
  ...
  void run(
    ...
    auto pool = memory::memoryManager()->addLeafPool(
        fmt::format("S3AsyncUploadBenchmark-{}", sequence));
    ...
    VELOX_CHECK_EQ(
        writeFile->size(), static_cast<uint64_t>(sizeMiB) * 1024 * 1024);
  }

 private:
  std::shared_ptr<const velox::config::ConfigBase> createConfig(
      bool enableUploadPartAsync) const {
    std::unordered_map<std::string, std::string> values;
    const auto endpoint = firstEnv({"GLUTEN_S3_BENCH_ENDPOINT", "AWS_ENDPOINT"});
    if (endpoint.has_value()) {
      values[s3ConfigKey(filesystems::S3Config::Keys::kEndpoint)] =
          endpoint.value();
    }

    if (auto accessKey = firstEnv(
            {"GLUTEN_S3_BENCH_ACCESS_KEY", "AWS_ACCESS_KEY_ID"})) {
      values[s3ConfigKey(filesystems::S3Config::Keys::kAccessKey)] =
          accessKey.value();
    }
    if (auto secretKey = firstEnv(
            {"GLUTEN_S3_BENCH_SECRET_KEY", "AWS_SECRET_ACCESS_KEY"})) {
      values[s3ConfigKey(filesystems::S3Config::Keys::kSecretKey)] =
          secretKey.value();
    }

    auto region = firstEnv(
        {"GLUTEN_S3_BENCH_REGION", "AWS_REGION", "AWS_DEFAULT_REGION"});
    if (!region.has_value()) {
      region = inferAwsRegion(endpoint);
    }
    if (region.has_value()) {
      values[s3ConfigKey(filesystems::S3Config::Keys::kEndpointRegion)] =
          region.value();
    }

    values[s3ConfigKey(filesystems::S3Config::Keys::kPathStyleAccess)] =
        getEnv("GLUTEN_S3_BENCH_PATH_STYLE_ACCESS")
            .value_or(boolString(defaultPathStyleAccess(endpoint)));
    values[s3ConfigKey(filesystems::S3Config::Keys::kSSLEnabled)] =
        getEnv("GLUTEN_S3_BENCH_SSL_ENABLED")
            .value_or(boolString(!endpoint.has_value() ||
                                 !startsWith(endpoint.value(), "http://")));
    values[s3ConfigKey(filesystems::S3Config::Keys::kMultipartMinPartSize)] =
        minPartSize_;
    values["hive.s3.part-upload-async"] =
